...
public class Client implements Runnable, PacketOutput {
	...
	public Client(Channel channel) throws IOException {
		...
		_log.debug(String.format("(%s) Login detected. Current memory:%d MB RAM, CurrentThreads=%d, "
	...
	public void handleDisconnect() {
		...
		if (_kick < 1) {
			...
			_log.debug("Client thread ended: " + getAccountName() + ":" + _hostname + " Current Memory: "
	...
	public synchronized void run() {
		...
		try {
			...
			if (elapse > longest) {
				String charName = _activeChar != null ? _activeChar.getName() : "unknown";
				_log.debug("Slow packet: opcode=" + opcode + " elapse=" + elapse + "ms char=" + charName + " (new longest)");
				longest = elapse;
			} else if (elapse > Config.SLOW_PACKET_DELAY) {
				String charName = _activeChar != null ? _activeChar.getName() : "unknown";
				_log.debug("Slow packet: opcode=" + opcode + " elapse=" + elapse + "ms char=" + charName);
			}
			return;
		} catch (Exception e) {
			_log.error("", e);
	...
	public void sendPacket() {
		try {
			...
			if (packet != null) {
				...
				if (abyte0.length < 4) {
					_log.debug("Bad/empty packet: " + packet.getClass().getName() + " length=" + abyte0.length);
					return;
				}
